...
namespace MicroLoom.Home
{
    public partial class Home1 : System.Web.UI.Page
    {
        ...
        void load_details(int u_id)
        {
            string strConnectionString = ConfigurationManager.ConnectionStrings["connection1"].ConnectionString;
            using (SqlConnection connection = new SqlConnection(strConnectionString))
            {
                connection.Open();
                SqlCommand command = new SqlCommand(null, connection);
                command.CommandText = "SELECT * FROM users WHERE u_id = @id";
                ...
                while (sdr.Read())
                {
                    lbl_info.Text = "Welcome " + sdr["full_name"].ToString();
                }
        ...
        void load_posts(long user_id)
        {
            ...
            try
            {
                ob.CreateConnection();
                ob.OpenConnection();
                ob._sqlCommand.CommandText = "get_home_page_posts";
                ob._sqlCommand.CommandType = CommandType.StoredProcedure;
                ob._sqlCommand.Parameters.AddWithValue("@current_user", user_id);
