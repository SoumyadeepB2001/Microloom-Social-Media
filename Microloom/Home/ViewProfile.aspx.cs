    ...
    public partial class ViewProfile : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                // Check if u_id is empty or null
                ...
                long u_id = Convert.ToInt64(Server.UrlDecode(Request.QueryString["u_id"]));
                long current_user_id = Convert.ToInt64(Session["current_user"].ToString());
        ...
        void load_profile_info(long user_id)
        {
            ...
            try
            {
                ...
                con.OpenConnection();
                con._sqlCommand.CommandText = "SELECT *, CASE WHEN EXISTS (SELECT * FROM follows WHERE following_id = " + user_id + " and follower_id = " + Session["current_user"].ToString() + ") THEN 'TRUE' ELSE 'FALSE' END AS FollowText FROM users WHERE u_id = " + user_id;
                con._sqlCommand.CommandType = CommandType.Text;

                SqlDataReader sdr = con._sqlCommand.ExecuteReader();

                if (!sdr.HasRows)  //Check if the reader has any rows
                {
                ...
                while (sdr.Read())
                {
                    ...
                    if (sdr["FollowText"].ToString() == "TRUE")
                    {
                        btn_follow.Width = 180;
        ...
        {
            long user_id = Convert.ToInt64(Server.UrlDecode(Request.QueryString["u_id"]));
        ...
        void load_comments(Repeater comments_repeater_, String p_id, long u_id)
        {
            ...
            try
            {
                ob.CreateConnection();
                ob.OpenConnection();
                ob._sqlCommand.CommandText = "select comment, users.u_id, username, profile_pic from comments, users where comments.p_id = " + p_id + " and comments.u_id = " + u_id + " and comments.u_id = users.u_id order by comment_time desc";
                ob._sqlCommand.CommandType = CommandType.Text;
                SqlDataAdapter sda = new SqlDataAdapter(ob._sqlCommand);
                DataSet ds = new DataSet();
                sda.Fill(ds);
        ...
        {
            string status = "";
            long profile_id = Convert.ToInt64(Server.UrlDecode(Request.QueryString["u_id"]));
            ...
            try
            {
                ob.CreateConnection();
                ob.OpenConnection();
                ob._sqlCommand.CommandText = "add_remove_follow";
                ob._sqlCommand.CommandType = CommandType.StoredProcedure;
                ob._sqlCommand.Parameters.AddWithValue("@status", status);
                ob._sqlCommand.Parameters.AddWithValue("@profile_id", profile_id);
                ob._sqlCommand.Parameters.AddWithValue("@user_id", Convert.ToInt32(Session["current_user"].ToString()));
                ob._sqlCommand.ExecuteNonQuery();
            }
